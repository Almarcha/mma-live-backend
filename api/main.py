...
==================================
Endpoints REST para la PWA.
Autenticación delegada a Supabase Auth (JWT).

Instalación:
    pip install fastapi uvicorn supabase python-dotenv python-jose
...
# CORS — permite peticiones desde la PWA
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "https://TU_USUARIO.github.io",   # ← Cambia esto
        "https://mmalive.netlify.app",     # ← Y esto
        "http://localhost:3000",
        "http://localhost:5500",
        "*",  # En producción, quita esto y pon solo tus dominios
    ],
...
SUPABASE_KEY     = os.getenv("SUPABASE_ANON_KEY", "")     # anon key para API pública
SUPABASE_SVC_KEY = os.getenv("SUPABASE_SERVICE_KEY", "")  # service key para admin
...
async def get_current_user(authorization: Optional[str] = Header(None)):
    """Extrae y verifica el usuario del token JWT."""
...
async def register(body: RegisterRequest):
    ...
    try:
        res = sb.auth.sign_up({
            ...
            "options": {
                "data": {"full_name": body.username or body.email.split("@")[0]}
            }
        })
        if res.user:
            # El trigger de Supabase crea user_profiles automáticamente
            if body.username:
...
async def get_my_profile(user=Depends(get_current_user)):
    """Obtener mi perfil."""
    sb = get_sb()
    res = sb.table("user_profiles").select("*, countries(name,flag_emoji)").eq(
        "id", str(user.id)
    ).execute()
    if not res.data:
...
async def get_my_favorites(user=Depends(get_current_user)):
    """Mis peleadores favoritos."""
    sb = get_sb()
    res = sb.table("user_favorite_fighters").select(
        "*, fighters(id, first_name, last_name, nickname, status, wins, losses, draws, profile_image_url)"
...
async def list_fighters(
    ...
    status:       Optional[str] = Query(None, description="Active | Retired"),
    weight_class: Optional[str] = Query(None, description="Nombre de la categoría"),
    gender:       Optional[str] = Query(None, description="M | F"),
    page:         int = Query(1, ge=1),
    per_page:     int = Query(20, ge=1, le=100),
):
    """Listar peleadores con filtros y paginación."""
    sb = get_sb()
...
async def get_fighter(slug: str, user=Depends(get_optional_user)):
    """Perfil completo de un peleador con historial de combates."""
    sb = get_sb()
    ...
    fighter_res = sb.table("fighters").select(
        ...
        "birth_country:countries!birth_country_id(name, flag_emoji), "
    ...
    rankings_res = sb.table("rankings").select(
        "rank_position, rank_date, "
        "weight_class:weight_classes(name_es)"
...
async def list_events(
    status:   Optional[str] = Query(None, description="Upcoming | Completed | Live"),
    page:     int = Query(1, ge=1),
    ...
    query = query.order("event_date", desc=(status == "Completed")).range(offset, offset + per_page - 1)
    res = query.execute()
...
async def upcoming_events():
    """Próximos eventos con cartelera resumida."""
    sb = get_sb()
...
async def get_event(slug: str):
    """Evento completo con todos los combates."""
    sb = get_sb()

    event_res = sb.table("events").select("*, country:countries(name, flag_emoji)").eq(
        "ufc_slug", slug
    ).execute()
    ...
    fights_res = sb.table("fights").select(
        "*, "
        "fighter_a:fighters!fighter_a_id(id, first_name, last_name, nickname, wins, losses, draws, profile_image_url, nationality:countries!nationality_id(flag_emoji)), "
        "fighter_b:fighters!fighter_b_id(id, first_name, last_name, nickname, wins, losses, draws, profile_image_url, nationality:countries!nationality_id(flag_emoji)), "
...
async def fight_prediction_stats(fight_id: str):
    """Estadísticas públicas de predicciones para un combate."""
    sb = get_sb()
...
async def create_prediction(
    ...
    """Crear predicción para un combate."""
    sb = get_sb()

    # Verificar que el combate es futuro
    fight = sb.table("fights").select("status, event_id").eq("id", fight_id).execute()
    if not fight.data:
...
async def get_rankings(weight_class_name: str):
    """Rankings actuales de una división."""
    sb = get_sb()
...
async def all_rankings():
    """Rankings de todas las divisiones."""
    sb = get_sb()
    res = sb.table("current_rankings").select("*").order("weight_class").order("rank_position").execute()
    return res.data
